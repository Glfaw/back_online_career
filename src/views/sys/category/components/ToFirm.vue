  ...
  <el-row :gutter="10">
    <el-col :span="6">
      <el-card shadow="never" header="搜索类型">
        <el-form :model="firmSearch">
          <el-form-item>
            <el-input clearable prefix-icon="" placeholder="请输入类型编号" v-model="firmSearch.id"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input clearable prefix-icon="" placeholder="请输入类型名称" v-model="firmSearch.name"></el-input>
          </el-form-item>
    ...
    <el-dialog title="企业类型信息" width="500px" :visible.sync="dialogVisible" :close-on-click-modal="false" @close="dialogClose">
...
import { getFirmTypeTree, getFirmType, addFirmType, updateFirmType, delFirmTypeById, delFirmTypeList } from '@/api/category'
...
export default {
  ...
  methods: {
    ...
    async handleDelType(id) {
      try {
        const res = await delFirmTypeById(id)
        if (res.code === 200) {
    ...
    async handleDelTypeList() {
      try {
        const res = await delFirmTypeList(this.selectIds)
        if (res.code === 200) {
    ...
    async handleSubmit() {
      try {
        const res = this.addOrUpdate ? await addFirmType(this.dialogForm): await updateFirmType(this.dialogForm)
        if (res.code === 200) {
